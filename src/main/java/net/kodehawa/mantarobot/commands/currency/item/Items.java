...
public class Items {
    ...
    public static final Item[] ALL = {
            HEADPHONES = new Item(ItemType.COLLECTABLE, "\uD83C\uDFA7", "Headphones", "items.headphones", "That's what happens when you listen to too much music. Should be worth something, tho.", 5, true, false),
            BAN_HAMMER = new Item(ItemType.COLLECTABLE, "\uD83D\uDD28", "Ban Hammer", "items.ban_hammer", "Left by an admin. +INF Dmg", 15, false),
            KICK_BOOT = new Item(ItemType.COLLECTABLE, "\uD83D\uDC62", "Kick Boot",  "items.kick_boot","Left by an admin. +INF Knockback", 12, false),
            FLOPPY_DISK = new Item(ItemType.COLLECTABLE, "\uD83D\uDCBE", "Floppy Disk", "items.floppy", "Might have some games.", 13, false),
            MY_MATHS = new Item(ItemType.COLLECTABLE, "\uD83D\uDCDD", "My Maths", "items.maths", "\"Oh, I forgot my maths.\"", 11, false),
            PING_RACKET = new Item(ItemType.COLLECTABLE, "\uD83C\uDFD3", "Ping Racket", "items.racket", "I won the game of ping-pong with Discord by a few milliseconds.", 15, false),
            LOADED_DICE = new Item(ItemType.COLLECTABLE, "\uD83C\uDFB2", "Loaded Die", "items.die", "Stolen from `~>roll` command", 45, false),
            FORGOTTEN_MUSIC = new Item(ItemType.COLLECTABLE, "\uD83C\uDFB5", "Forgotten Music", "items.forgotten", "Never downloaded. Probably has been copyrighted.", 15, false),
            CC_PENCIL = new Item(ItemType.COLLECTABLE, "\u270f", "Pencil", "items.pencil", "We have plenty of those!", 15, false),
            OVERFLOWED_BAG = new Item(ItemType.COLLECTABLE, "\uD83D\uDCB0","Moneybag", "items.moneybag", "What else?.", 95, true),
            BROM_PICKAXE = new Item(ItemType.INTERACTIVE, "\u26cf","Brom's Pickaxe", "items.pick", "That guy liked Minecraft way too much. (`~>mine` tool)", 100, true),
            MILK = new Item(ItemType.COMMON, EmoteReference.POTION1.getUnicode(),"Milk", "items.milk", "Maybe it's okay to have some.", 25, true),
            ALCOHOL = new Item(ItemType.COMMON, EmoteReference.POTION2.getUnicode(),"Beverage", "items.beverage", "Does really weird stuff in big quantities.", 25, true),
            LEWD_MAGAZINE = new Item(ItemType.COMMON, EmoteReference.MAGAZINE.getUnicode(),"Lewd Magazine", "items.lewd", "Too many lewd commands.", 25, true),
            RING = new Item(ItemType.COMMON, EmoteReference.RING.getUnicode(),"Marriage Ring", "items.ring", "Basically what makes your marriage official", 60, true),
            LOVE_LETTER = new Item(ItemType.COLLECTABLE, EmoteReference.LOVE_LETTER.getUnicode(),"Love Letter", "items.letter", "A letter from your beloved one.", 45, false),
            LOOT_CRATE_KEY = new Item(ItemType.COMMON, EmoteReference.KEY.getUnicode(),"Crate Key", "items.key", "Used to open loot boxes with `~>opencrate` or `~>useitem loot crate`", 58, true),
            CLOTHES = new Item(ItemType.COMMON, EmoteReference.CLOTHES.getUnicode(),"Clothes", "items.clothes", "Basically what you wear.", 30, true),
            DIAMOND = new Item(ItemType.COMMON, EmoteReference.DIAMOND.getUnicode(),"Diamond", "items.diamond", "Basically a better way of saving your money. It's shiny too.", 350, true),
            DRESS = new Item(ItemType.COMMON, EmoteReference.DRESS.getUnicode(),"Wedding Dress", "items.dress", "Isn't it cute?", 75, true),
            NECKLACE = new Item(ItemType.COMMON, EmoteReference.NECKLACE.getUnicode(),"Necklace", "items.necklace", "Looks nice.", 17, true),
            TUXEDO = new Item(ItemType.COMMON, EmoteReference.TUXEDO.getUnicode(),"Tuxedo", "items.tuxedo", "What you wear when you're going to get married with a girl.", 50, true),
            SHOES = new Item(ItemType.COMMON, EmoteReference.SHOES.getUnicode(),"Shoes", "items.shoes", "Cause walking barefoot is just nasty.", 10, true),
            ROSE = new Item(ItemType.COMMON, EmoteReference.ROSE.getUnicode(),"Rose", "items.rose", "The embodiment of your love.", 25, true),
            CHOCOLATE = new Item(ItemType.COMMON, EmoteReference.CHOCOLATE.getUnicode(),"Chocolate", "items.chocolate", "Yummy.", 23, true),
            COOKIES = new Item(ItemType.COMMON, EmoteReference.COOKIE.getUnicode(),"Cookie", "items.cookie", "Delicious.", 10, true),
            ...
            STAR = new Item(ItemType.COLLECTABLE, "\u26A0\uFE0F","Prize", "items.prize", "Pretty much, huh? `Leftover from Currency version 1. No longer obtainable.`", 0, false, false, true),

            // ---------------------------------- LEFT OVERS FROM CURRENCY V1 END HERE ----------------------------------
            LOOT_CRATE = new Item(ItemType.INTERACTIVE, EmoteReference.LOOT_CRATE.getDiscordNotation(),"Loot Crate",  "items.crate","You can use this along with a loot key to open a loot crate! `~>opencrate`", 0, false, false, true, Items::openLootCrate),
            STAR_2 = new Item(ItemType.COMMON, EmoteReference.STAR.getUnicode(),"Prize 2", "items.prize_2", "In the first place, how did you get so much money?", 500, true, false, true),
            SLOT_COIN = new Item(ItemType.COMMON, "\uD83C\uDF9F","Slot ticket", "items.slot_ticket","Gives you extra chance in slots, also works as bulk storage.", 65, true, true),
            HOUSE = new Item(ItemType.COMMON, EmoteReference.HOUSE.getUnicode(), "House", "items.house", "Cozy place to live in.", 5000, true, true),
            CAR = new Item(ItemType.COMMON, "\uD83D\uDE97","Car",  "items.car","To move around.", 1000, true, true),

            // ---------------------------------- CHRISTMAS 2017 EVENT STARTS HERE ----------------------------------
            BELL_SPECIAL = new Item(ItemType.RARE, "\uD83D\uDD14","Christmas bell", "items.bell", "Christmas event 2017 reward. Gives you a cozy christmas feeling on your tree.", 0, false, false, true),
            CHRISTMAS_TREE_SPECIAL = new Item(ItemType.RARE, "\uD83C\uDF84", "Christmas tree", "items.tree", "Christmas event 2017 reward. Who doesn't like a christmas tree?.", 0, false, false, true),
            // ---------------------------------- CHRISTMAS 2017 EVENT ENDS HERE ----------------------------------

            // ---------------------------------- 5.0 ITEMS START HERE ----------------------------------
            PANTS = new Item(ItemType.COMMON, "\uD83D\uDC56", "Pants", "items.pants", "Basically what you wear on your legs... hopefully.", 20, true),
            POTION_HASTE = new Item(ItemType.INTERACTIVE, "\uD83C\uDF76","Haste Potion", "items.haste", "Allows you to collect more stuff from lootable commands for 2 sessions.", 490, true),
            POTION_CLEAN = new Item(ItemType.INTERACTIVE, "\uD83C\uDF7C","Milky Potion", "items.milky", "Clears all potion effects.", 50, true),
            POTION_STAMINA = new Item(ItemType.INTERACTIVE, "\uD83C\uDFFA","Energy Beverage", "items.energy", "Gives less chance of a pick breaking while mining. Lasts only 5 mining sessions.", 450, true),
            FISHING_ROD = new Item(ItemType.INTERACTIVE, "\uD83C\uDFA3","Fishing Rod", "items.rod", "Enables you to fish.", 65, true),
            FISH_1 = new Item(ItemType.FISHING, "\uD83D\uDC1F","Fish", "items.fish", "Common Fish. Caught in fishing", 10, false),
            FISH_2 = new Item(ItemType.FISHING, "\uD83D\uDC20","Tropical Fish", "items.tropical_fish", "A very tropical fish. Caught in fishing", 30, false),
            FISH_3 = new Item(ItemType.FISHING, "\uD83D\uDC21","Blowfish", "items.blowfish", "I wonder... is this good?. Caught in fishing.", 15, false),
            // ---------------------------------- 5.0 MINING ITEMS START HERE ----------------------------------
            GEM_1 = new Item(ItemType.MINE, "\u2604", "Comet Gem", "items.comet_gem", "Fragments of a comet you found while mining. Useful for casting.", 40, false),
            GEM_2 = new Item(ItemType.MINE, EmoteReference.STAR.getUnicode(), "Star Gem", "items.star_gem", "Fragments of a fallen star you found while mining.", 60, false),
            GEM_3 = new Item(ItemType.MINE, "\uD83D\uDD78", "Cobweb", "items.cobweb", "Something a spider left over on the mine. Wonder if it's worth something.", 10, false),
            GEM_4 = new Item(ItemType.MINE, "\uD83D\uDCAB", "Gem Fragment", "items.fragment", "Fragment of an ancient gem. Useful for casting", 50, false),
            // ---------------------------------- 5.0 ITEMS START HERE (again lol) ----------------------------------
            MOP = new Item(ItemType.INTERACTIVE, "\u3030","Mop", "items.mop", "A delightful way to clean all the dust you have around.", 10, true),
            CLAIM_KEY = new Item(ItemType.COMMON, EmoteReference.KEY.getUnicode(),"Claim Key", "items.claim_key", "This items makes you unclaimeable (as a waifu) while having it on your inventory.", 1, false, true),
            COFFEE = new Item(ItemType.COMMON, "\u2615","Coffee", "items.coffee", "A delightful way to start your day.", 10, true),
            WAIFU_PILL = new Item(ItemType.INTERACTIVE, "\ud83d\udc8a","Waifu Pill", "items.waifu_pill", "Gives you a significant advantage on mine and fish if one of your waifus is valued at over 2 million. Lasts 5 sessions.", 670, true),
            FISHING_BAIT = new Item(ItemType.INTERACTIVE, "\uD83D\uDC1B","Fishing bait.", "items.bait", "Gives you a higher chance of catching fish.", 15, true),
            DIAMOND_PICKAXE = new Item(ItemType.CAST_MINE, "\u2692\ufe0f","Diamond Pickaxe", "items.diamond_pick", "A very much stronger pick (Diamond). (`~>mine` tool)", 450, true, false, "1;2", 10, 18),
            TELEVISION = new Item(ItemType.COMMON, "\uD83D\uDCFA","Television", "items.tv", "Must... watch... TV...", 45, true),
            WRENCH = new Item(ItemType.COMMON, "\ud83d\udd27","Wrench", "items.wrench", "Casting tool. Useful to put stuff together.", 50, true),
            //car is 1000 credits, so this is 350
            MOTORCYCLE = new Item(ItemType.COMMON, "\uD83C\uDFCD","Motorcycle",  "items.headphones","Going around, the cool way.", 350, true),
            //TODO: proper emojis
            GEM1_PICKAXE = new Item(ItemType.CAST_MINE, "\u2692\ufe0f","Comet Gem Pickaxe", "items.comet_pick", "A very much stronger pick (Comet). (`~>mine` tool)", 350, true, false, "1;2", 10, 48),
            GEM2_PICKAXE = new Item(ItemType.CAST_MINE, "\u2692\ufe0f","Star Gem Pickaxe", "items.star_pick", "A very much stronger pick (Star). (`~>mine` tool)", 350, true, false, "1;2", 10, 49),
    };
