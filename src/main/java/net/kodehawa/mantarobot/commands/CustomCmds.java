...
public class CustomCmds {
    ...
    public static class Custom extends SlashCommand {
        ...
        // Would need some magic to fix this lol
        // Issue is: no multiline on slash commands yet, and custom commands hard-require an event for some parsing.
        // TODO: either port fully or add warning to use add/remove/eval/edit on prefix commands only for now.
        /*
        ...
        @Options({
                @Options.Option(type = OptionType.STRING, name = "eval", description = "The content to evaluate.", required = true)
        })
    ...
    private static void listCustoms(IContext ctx) {
        ...
        String description = ctx.getLanguageContext().get("general.dust");
        if (!commands.isEmpty()) {
    ...
    private static void rawCommand(IContext ctx, String command) {
        ...
        EmbedBuilder embed = ctx.baseEmbed(
    ...
    public static void deleteResponseCmd(IContext ctx, String name, int where) {
        ...
        CustomCommand custom = ctx.db().getCustomCommand(ctx.getGuild(), name);
        if (custom == null) {
