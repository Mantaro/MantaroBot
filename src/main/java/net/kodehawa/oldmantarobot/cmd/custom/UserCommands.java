package net.kodehawa.oldmantarobot.cmd.custom;
...
public class UserCommands extends Module {
	...
	public UserCommands() {
		...
		Mantaro.schedule(() -> {
			...
			custom.values().forEach(map -> map.keySet().forEach(cmd -> {
				if (!Manager.commands.containsKey(cmd)) Manager.commands.put(cmd, customCommand);
	...
	public void registerCommands() {
		super.register("addcustom", new SimpleCommand() {
			@Override
			public void onCommand(String[] args, String content, GuildMessageReceivedEvent event) {
				if (!content.startsWith("debug")) {
					...
					if (Manager.commands.containsKey(name) && Manager.commands.get(name) != customCommand) {
					...
					if (!Manager.commands.containsKey(name)) Manager.commands.put(name, customCommand);
		...
		super.register("deletecustom", new SimpleCommand() {
			@Override
			public String help() {
		...
		super.register("listcustom", new SimpleCommand() {
			@Override
			public String help() {
