...
public class BirthdayListener extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
                    ...
                    if(!Birthday.bd.get(userKey).isEmpty()){
                        try{
                            ...
                            if(Birthday.bd.get(userKey).substring(0, 5).equals(format1.format(cal.getTime()).substring(0, 5))) {
                                ...
                                List<Member> user1 = event.getGuild().getMembers();
                                for (Member finalMember1 : user1) {
                                    if(guild.getMembersWithRoles(birthdayRole1).contains(finalMember1)){
                                        guild.getController().removeRolesFromMember(finalMember1, birthdayRole1).queue();
                                    }
                                }
                            }
                        } catch(Exception e){
        ...
        AsyncHelper.instance().asyncThread("Birthday Listener", r).run();
    }
}
