	...
	public void onMessageDelete(MessageDeleteEvent event){
		try{
			...
			if(Parameters.getLogHash().containsKey(event.getGuild().getId())){
				...
				if(!deletedMessage.getContent().isEmpty()){
	...
	public void onMessageUpdate(MessageUpdateEvent event){
		try{
			...
			if(Parameters.getLogHash().containsKey(event.getGuild().getId()) && !event.getAuthor().isBot() /*So it doesnt log when I edit the message*/){
				...
				if(!editedMessage.getContent().isEmpty()){
				    tc.sendMessage(":warning: `[" + hour + "]` " + author.getName()+ "#" + author.getDiscriminator() + " *modified* a message" + ".\n" + "**Previous content:** " + editedMessage.getContent() + "\n**New content:** "
