...
public class Perms extends Command {
	...
	public void onCommand(String[] split, String content, MessageReceivedEvent event) {
		if(!event.getAuthor().getId().equals(Mantaro.OWNER_ID)){
		...
		else if(guild.getMember(author).hasPermission(Permission.ADMINISTRATOR)){
			List<User> mentions = event.getMessage().getMentionedUsers();
			String userId = null;
			for(User s : mentions){
				userId = s.getId(); 
			...
			if(split[1].equals("user") || split[1].equals("admin") && userId != null){
				...
				event.getChannel().sendMessage("Permission level specified isn't valid or you mentioned no user or you have no permissions to do this.\r\n Valid levels: user, admin, owner").queue();
			}
		}
	}
